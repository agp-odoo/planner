odoo.define('planner.renderer', function (require) {
    "use strict";
    ...
    const RenderListContent = FieldMany2ManyTags.extend({
        ...
        _parsedContent: function () {
            return this._getRenderTagsContext().map(
                elm => this._parseText(elm.content, elm.id) + ' '
            ).join(' ');
        },
    });

    registry.add("render_content", RenderListContent);
